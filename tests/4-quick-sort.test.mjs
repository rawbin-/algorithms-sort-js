...
describe('quick sort', () => {
    test.each(testData)('quickSortA-0 arg-$#',({input,output}) => {
        expect(quickSortA(input)).toEqual(output)
    })

    test.each(testData)('quickSortA-1 arg-$#',({input,output}) => {
        expect(quickSortA(input,0)).toEqual(output)
    })

    test.each(testData)('quickSortA-2 arg-$#',({input,output}) => {
        expect(quickSortA(input,0,input && input.length - 1)).toEqual(output)
    })

    test.each(testData)('quickSortB-0 arg-$#',({input,output}) => {
        expect(quickSortB(input)).toEqual(output)
        expect(quickSortB(input, 0)).toEqual(output)
        expect(quickSortB(input, 0 , input && input.length - 1)).toEqual(output)
    })
